...
std::unordered_set<std::tuple<char, char, char>, hash_tuple> make_unique_trigrams(const std::string& s) {
    std::unordered_set<std::tuple<char, char, char>, hash_tuple> trigrams;
    if (s.size() >= 3) {
        for(auto it = s.cbegin(); it != std::prev(std::prev(s.cend())); ++it) {
            trigrams.insert(std::make_tuple(*it, *std::next(it), *std::next(std::next(it))));
        }
    }
...
std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> top_query_ngrams2(const std::vector<std::string> & regexes, int k) {

    // count number of occurence for all ngrams
    std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> ngram_counts;
    std::vector<std::tuple<char, char, char>> keys;
    
    for (const auto & reg_string : regexes) {
        auto curr_literals = std::get<0>(split_regex_multi(reg_string));
        std::unordered_set<std::tuple<char, char, char>, hash_tuple> curr_grams;
        for (const auto & lit : curr_literals) {
            insert_unique_trigrams_to_set(curr_grams, lit);
        }
    ...
    std::sort(
        keys.begin(), 
        keys.end(),
        [&ngram_counts](std::tuple<char, char, char> const& a, std::tuple<char, char, char> const& b) {
            if (a == b) {
                return false;
            }
    ...
    std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> result;
...
std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> top_query_ngrams(const std::vector<std::string> & regex_lits, int k) {

    // count number of occurence for all ngrams
    std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> ngram_counts;
    std::vector<std::tuple<char, char, char>> keys;
    for (const auto & line : regex_lits) {
        std::unordered_set<std::tuple<char, char, char>, hash_tuple> curr_grams = make_unique_trigrams(line);
        for (const auto & line_gram: curr_grams) {
            auto it = ngram_counts.find(line_gram);
            if (it != ngram_counts.end()) {
                ngram_counts[line_gram]++;
...
// return a vector of m-bits array, each corresponds to existence of some trigrams in the log
std::vector<std::bitset<NUM_INDEXED>> build_index(const std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> & gram_to_idx, const std::vector<std::string> & lines) {

    std::vector<std::bitset<NUM_INDEXED>> result;
    for (const auto & line : lines) {
        // init a bitset of all 0's
        std::bitset<NUM_INDEXED> curr_bitarr;
        auto curr_grams = make_unique_trigrams(line);
        for (const auto & line_gram: curr_grams) {
            auto it = gram_to_idx.find(line_gram);
            if (it != gram_to_idx.end()) {
                // trigram exits, set bit at the index
...
std::unordered_map<std::tuple<char, char, char>, std::vector<unsigned int>, hash_tuple> build_inverted_index(const std::vector<std::string> & regex_lits, const std::vector<std::string> & lines) {
    // create an entry for each ngram in the regex
    std::unordered_map<std::tuple<char, char, char>, std::vector<unsigned int>, hash_tuple> ngram_inverted;
    for (const auto & line : regex_lits) {
        std::unordered_set<std::tuple<char, char, char>, hash_tuple> curr_grams = make_unique_trigrams(line);
        for (const auto & line_gram: curr_grams) {
            if (ngram_inverted.find(line_gram) == ngram_inverted.end()) {
                // ngram_inverted.emplace(std::tuple<char, char, char>, std::vector<unsigned int>());
    ...
    for (size_t idx = 0; idx < lines.size(); idx++) {
        ...
        for (const auto & line_gram: curr_grams) {
            ...
            if (it != ngram_inverted.end()) {
                // trigram exits, add line idx to the inverted index
...
std::unordered_map<std::tuple<char, char, char>, std::vector<unsigned int>, hash_tuple> build_topk_inverted_index(const std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> & gram_to_idx, const std::vector<std::string> & lines) {
...
std::bitset<NUM_INDEXED> GetBitMask(const std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> & gram_to_idx, const std::string & reg_string) {
    // get the bit mask for the regex
    std::vector<std::string> reg_lits = std::get<0>(split_regex_multi(reg_string));
    std::unordered_set<std::tuple<char, char, char>, hash_tuple> reg_trigrams{};
    for (const auto & lit : reg_lits) {
        reg_trigrams.merge(make_unique_trigrams(lit));
    }
    std::bitset<NUM_INDEXED> bitmask;
    bitmask.set();
    // start with all 1's
    for (const auto & reg_gram: reg_trigrams) {
        auto it = gram_to_idx.find(reg_gram);
        if (it != gram_to_idx.end()) {
            // trigram exits, unset bit at the index
...
std::pair<double, int> Re2FilterMatch(const std::vector<std::string> & lines, const std::vector<std::bitset<NUM_INDEXED>> & idxs, const std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> & gram_to_idx, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
...
std::pair<double, int> Re2InvertedFilterMatch1(const std::vector<std::string> & lines, std::unordered_map<std::tuple<char, char, char>, std::vector<unsigned int>, hash_tuple> ngram_inverted, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
    auto curr_literals = std::get<0>(split_regex_multi(reg_string));
    std::unordered_set<std::tuple<char, char, char>, hash_tuple> unique_trigrams{};
    for (const auto & curr_lit : curr_literals) {
        unique_trigrams.merge(make_unique_trigrams(curr_lit));
    }

    for (auto i = 0; i < lines.size(); i++) {
        for (const auto & ngram : unique_trigrams) {
...
std::pair<double, int> Re2InvertedFilterMatch2(const std::vector<std::string> & lines, std::unordered_map<std::tuple<char, char, char>, std::vector<unsigned int>, hash_tuple> ngram_inverted, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
    ...
    // find shortest
    std::vector<std::tuple<char, char, char>> checking_ngram{};
    std::tuple<char, char, char> shortest_ngram;
    int shortest_size = -1;
    bool first = true;
...
int StatsAfterFiltering(const std::vector<std::string> & lines, const std::vector<std::bitset<NUM_INDEXED>> & idxs, const std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> & gram_to_idx, const std::string & reg_string) {
    auto bitmask = GetBitMask(gram_to_idx, reg_string);
    int count = 0;
    for (auto i = 0; i < lines.size(); i++) {
        // check if the indexed ngrams in regular expressions are present in the current log line
...
unsigned long long calculate_map_size(const std::unordered_map<std::tuple<char, char, char>, std::vector<unsigned int>, hash_tuple> & inv_idx, bool max) {
    unsigned long long entrySize = sizeof(std::tple<char, char, char>) + sizeof(void*);
...
int main(int argc, char** argv) {
    ...
    // std::string reg_file = "db_x-cpp-log-format-strings-as-regexp.csv";
    ...
    std::unordered_map<std::tuple<char, char, char>, size_t, hash_tuple> gram_to_idx = top_query_ngrams2(regexes, NUM_INDEXED);
    auto start = std::chrono::high_resolution_clock::now();
    ...
    std::unordered_map<std::tuple<char, char, char>, std::vector<unsigned int>, hash_tuple> inverted_index = build_topk_inverted_index(gram_to_idx, lines);
    ...
    ind_out << "trigram," << NUM_INDEXED << "," << bit_index_building_time.count() << "," << bit_index_size << "," << inverted_index_building_time.count() << "," << inverted_index_size_fit << "," << inverted_index_size_max << std::endl;
    ind_out.close();


    int num_repeat = 10;

    const std::filesystem::path dir_path = "ngram_record_all_script/new_trigram_" + std::to_string(NUM_INDEXED);
    ...
    for (size_t i = 0; i < num_repeat; i++) {
        std::ofstream expr_r_file{dir_path/("trigram_" + std::string(std::string(argv[1])) + "_" + std::to_string(i) + ".csv"), std::ofstream::out};
...
// g++ -O3 -std=c++17   -Ofast -march=native -mfma -mavx -fomit-frame-pointer -ffp-contract=fast -flto -DARMA_NO_DEBUG -pthread trigram_record_all.cpp  -L/usr/local/lib/ -lre2 -lstdc++fs -o trigram_record_all.o