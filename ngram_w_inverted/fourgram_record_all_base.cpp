...
struct hash_4tuple {
    ...
    {
        return hash_val(std::get<0>(p), std::get<1>(p), std::get<2>(p), std::get<3>(p));
...
std::vector<std::string> read_db_x() {
    ...
    for (const auto & entry : std::filesystem::directory_iterator(path)) {
        ...
        //  101876733 
        while (getline(data_in, line)){
        // while (lines.size() < 1000000 && getline(data_in, line)){ // 11.6657s index building
            line.erase(std::remove(line.begin(), line.end(), '\r'), line.end());
            if (line.size() > 2)
...
std::unordered_set<std::tuple<char, char, char, char>, hash_4tuple> make_unique_fourgrams(const std::string& s) {
    std::unordered_set<std::tuple<char, char, char, char>, hash_4tuple> grams;

    if (s.size() >= 4) {
        for(auto it = s.cbegin(); it != s.cend() - 4; ++it) {
            grams.insert(std::make_tuple(*it, *(it+1), *(it+2), *(it+3)));
        }
    }
    return grams;
}

std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> top_query_ngrams(const std::vector<std::string> & regex_lits, int k) {

    // count number of occurence for all ngrams
    std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> ngram_counts;
    std::vector<std::tuple<char, char, char, char>> keys;
    for (const auto & line : regex_lits) {
        std::unordered_set<std::tuple<char, char, char, char>, hash_4tuple> curr_grams = make_unique_fourgrams(line);
    ...
    std::sort(
        keys.begin(), 
        keys.end(),
        [&ngram_counts](std::tuple<char, char, char, char> const& a, std::tuple<char, char, char, char> const& b) {
            if (a == b) {
                return false;
            }
    ...
    std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> result;
...
std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> top_query_ngrams2(const std::vector<std::string> & regexes, int k) {
    ...
    for (const auto & reg_string : regexes) {
        ...
        std::unordered_set<std::tuple<char, char, char, char>, hash_4tuple> curr_grams;
        for (const auto & lit : curr_literals) {
            insert_unique_fourgrams_to_set(curr_grams, lit);
        }
        for (const auto & line_gram: curr_grams) {
            auto it = ngram_counts.find(line_gram);
            if (it != ngram_counts.end()) {
...
// return a vector of m-bits array, each corresponds to existence of some fourgrams in the log
std::vector<std::bitset<NUM_INDEXED>> build_index(const std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> & gram_to_idx, const std::vector<std::string> & lines) {

    std::vector<std::bitset<NUM_INDEXED>> result;
    for (const auto & line : lines) {
        // init a bitset of all 0's
        std::bitset<NUM_INDEXED> curr_bitarr;
        auto curr_grams = make_unique_fourgrams(line);
        for (const auto & line_gram: curr_grams) {
            auto it = gram_to_idx.find(line_gram);
            if (it != gram_to_idx.end()) {
                // fourgram exits, set bit at the index
...
std::unordered_map<std::tuple<char, char, char, char>, std::vector<unsigned int>, hash_4tuple> build_inverted_index(const std::vector<std::string> & regex_lits, const std::vector<std::string> & lines) {
    // create an entry for each ngram in the regex
    std::unordered_map<std::tuple<char, char, char, char>, std::vector<unsigned int>, hash_4tuple> ngram_inverted;
    for (const auto & line : regex_lits) {
        std::unordered_set<std::tuple<char, char, char, char>, hash_4tuple> curr_grams = make_unique_fourgrams(line);
        for (const auto & line_gram: curr_grams) {
            if (ngram_inverted.find(line_gram) == ngram_inverted.end()) {
                // ngram_inverted.emplace(std::tuple<char, char, char, char>, std::vector<unsigned int>());
    ...
    for (size_t idx = 0; idx < lines.size(); idx++) {
        ...
        for (const auto & line_gram: curr_grams) {
            ...
            if (it != ngram_inverted.end()) {
                // fourgram exits, add line idx to the inverted index
...
std::unordered_map<std::tuple<char, char, char, char>, std::vector<unsigned int>, hash_4tuple> build_topk_inverted_index(const std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> & gram_to_idx, const std::vector<std::string> & lines) {
...
std::bitset<NUM_INDEXED> GetBitMask(const std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> & gram_to_idx, const std::string & reg_string) {
    // get the bit mask for the regex
    std::vector<std::string> reg_lits = std::get<0>(split_regex_multi(reg_string));
    std::unordered_set<std::tuple<char, char, char, char>, hash_4tuple> reg_fourgrams{};
    for (const auto & lit : reg_lits) {
        reg_fourgrams.merge(make_unique_fourgrams(lit));
    }
    std::bitset<NUM_INDEXED> bitmask;
    bitmask.set();
    // start with all 1's
    for (const auto & reg_gram: reg_fourgrams) {
        auto it = gram_to_idx.find(reg_gram);
        if (it != gram_to_idx.end()) {
            // fourgram exits, unset bit at the index
...
std::pair<double, int> Re2FilterMatch(const std::vector<std::string> & lines, const std::vector<std::bitset<NUM_INDEXED>> & idxs, const std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> & gram_to_idx, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
...
std::pair<double, int> Re2InvertedFilterMatch1(const std::vector<std::string> & lines, std::unordered_map<std::tuple<char, char, char, char>, std::vector<unsigned int>, hash_4tuple> ngram_inverted, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
    auto curr_literals = std::get<0>(split_regex_multi(reg_string));
    std::unordered_set<std::tuple<char, char, char, char>, hash_4tuple> unique_fourgrams{};
    for (const auto & curr_lit : curr_literals) {
        unique_fourgrams.merge(make_unique_fourgrams(curr_lit));
    }

    for (auto i = 0; i < lines.size(); i++) {
        for (const auto & ngram : unique_fourgrams) {
            // if current ngram is in the inverted index
            //   and there is a smallest element in the the corresponding vector of the ngram in the inverted index that is >= the current line index
            //   and the element != the current line index
            // then go to next line
            if (ngram_inverted.find(ngram) != ngram_inverted.end()) {
                auto it = std::lower_bound(ngram_inverted[ngram].begin(), ngram_inverted[ngram].end(), i);
                if ((it != ngram_inverted[ngram].end()) && (*it != i)) {
                    goto NEXT_LINE;
                }
            }
        }
    ...
    auto end = std::chrono::high_resolution_clock::now();
    auto elapsed_seconds = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);
    // std::cout << "Time1: " << elapsed_seconds.count() << " Count1 " << count << std::endl;
    return std::make_pair(elapsed_seconds.count(), count);
}

std::pair<double, int> Re2InvertedFilterMatch2(const std::vector<std::string> & lines, std::unordered_map<std::tuple<char, char, char, char>, std::vector<unsigned int>, hash_4tuple> ngram_inverted, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
    ...
    // find shortest
    std::vector<std::tuple<char, char, char, char>> checking_ngram{};
    std::tuple<char, char, char, char> shortest_ngram;
    int shortest_size = -1;
    bool first = true;
...
int StatsAfterFiltering(const std::vector<std::string> & lines, const std::vector<std::bitset<NUM_INDEXED>> & idxs, const std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> & gram_to_idx, const std::string & reg_string) {
    auto bitmask = GetBitMask(gram_to_idx, reg_string);
    int count = 0;
    for (auto i = 0; i < lines.size(); i++) {
        // check if the indexed ngrams in regular expressions are present in the current log line
...
unsigned long long calculate_map_size(const std::unordered_map<std::tuple<char, char, char, char>, std::vector<unsigned int>, hash_4tuple> & inv_idx, bool max) {
    unsigned long long entrySize = sizeof(std::tuple<char, char, char, char>) + sizeof(void*);
...
int main(int argc, char** argv) {
    ...
    std::unordered_map<std::tuple<char, char, char, char>, size_t, hash_4tuple> gram_to_idx = top_query_ngrams2(regexes, NUM_INDEXED);
    auto start = std::chrono::high_resolution_clock::now();
    auto top_idxs = build_index(gram_to_idx, lines);
    auto end = std::chrono::high_resolution_clock::now();
    ...
    std::unordered_map<std::tuple<char, char, char, char>, std::vector<unsigned int>, hash_4tuple> inverted_index = build_topk_inverted_index(gram_to_idx, lines);
    ...
    ind_out << "fourgram," << NUM_INDEXED << "," << bit_index_building_time.count() << "," << bit_index_size << "," << inverted_index_building_time.count() << "," << inverted_index_size_fit << "," << inverted_index_size_max << std::endl;
    ind_out.close();


    int num_repeat = 10;

    const std::filesystem::path dir_path = "ngram_record_all_script/new_fourgram_" + std::to_string(NUM_INDEXED);
    ...
    for (size_t i = 0; i < num_repeat; i++) {
        std::ofstream expr_r_file{dir_path/("fourgram_" + std::string(std::string(argv[1])) + "_" + std::to_string(i) + ".csv"), std::ofstream::out};
    ...
    r_file << "regex\tfilter_time\tmid_filter_time\tinverted_time\tmid_inverted_time\tdirect_time\tmid_direct_time\tfilter\tnum_after_filter\tmatch_num_filter\tmatch_num_inverted\tmatch_num_direct" << std::endl;
    ...
    for (const std::string & r : regexes) {
        ...
        for (int i = 0; i < num_repeat; i++) {
            std::pair<double, int> result_filter = Re2FilterMatch(lines, top_idxs, gram_to_idx, r);
            ...
            std::pair<double, int> result_inverted = Re2InvertedFilterMatch2(lines, inverted_index, r);
        ...
        r_file << bitmask.to_string() << "\t" << StatsAfterFiltering(lines, top_idxs, gram_to_idx, r) << "\t";
...
// g++ -O3 -std=c++17   -Ofast -march=native -mfma -mavx -fomit-frame-pointer -ffp-contract=fast -flto -DARMA_NO_DEBUG -pthread fourgram_record_all.cpp  -L/usr/local/lib/ -lre2 -lstdc++fs -o fourgram_record_all.o