...
std::vector<std::string> read_sys_y() {
    ...
    // while (lines.size() < 1000000 && getline(data_in, line)){
    while (getline(data_in, line)){
        line.erase(std::remove(line.begin(), line.end(), '\r'), line.end());
        std::stringstream streamData(line);
...
std::unordered_map<std::pair<char, char>, size_t, hash_pair> top_query_ngrams_threshold(const std::vector<std::string> & regexes, int k) {
    ...
    auto threshold = regexes.size()/10;
    ...
    size_t idx = 0;
    for (size_t key_idx = 0; key_idx < keys.size() && idx < NUM_INDEXED; key_idx++) {
        if (ngram_counts[keys[key_idx]] <= threshold) {
            result.insert({keys[key_idx], idx++});
        }
    }
    size_t key_idx = 0;
    while (result.size() < NUM_INDEXED && key_idx < keys.size()) {
        result.insert({keys[key_idx++], idx++});
    }
...
std::vector<std::bitset<NUM_INDEXED>> build_index(const std::unordered_map<std::pair<char, char>, size_t, hash_pair> & gram_to_idx, const std::vector<std::string> & lines) {
...
std::unordered_map<std::pair<char, char>, std::vector<unsigned int>, hash_pair> build_inverted_index(const std::vector<std::string> & regex_lits, const std::vector<std::string> & lines) {
...
std::unordered_map<std::pair<char, char>, std::vector<unsigned int>, hash_pair> build_topk_inverted_index(const std::unordered_map<std::pair<char, char>, size_t, hash_pair> & gram_to_idx, const std::vector<std::string> & lines) {
...
std::pair<double, int> Re2FilterMatch(const std::vector<std::string> & lines, const std::vector<std::bitset<NUM_INDEXED>> & idxs, const std::unordered_map<std::pair<char, char>, size_t, hash_pair> & gram_to_idx, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
...
std::pair<double, int> Re2InvertedFilterMatch1(const std::vector<std::string> & lines, std::unordered_map<std::pair<char, char>, std::vector<unsigned int>, hash_pair> ngram_inverted, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
...
std::pair<double, int> Re2InvertedFilterMatch2(const std::vector<std::string> & lines, std::unordered_map<std::pair<char, char>, std::vector<unsigned int>, hash_pair> ngram_inverted, const std::string & reg_string) {
    auto start = std::chrono::high_resolution_clock::now();
    int count = 0;
    std::string sm;
    RE2 reg(reg_string);
...
int StatsAfterFiltering(const std::vector<std::string> & lines, const std::vector<std::bitset<NUM_INDEXED>> & idxs, const std::unordered_map<std::pair<char, char>, size_t, hash_pair> & gram_to_idx, const std::string & reg_string) {
    auto bitmask = GetBitMask(gram_to_idx, reg_string);
    int count = 0;
    for (auto i = 0; i < lines.size(); i++) {
        // check if the indexed ngrams in regular expressions are present in the current log line
...
size_t calculate_map_size(const std::unordered_map<std::pair<char, char>, std::vector<unsigned int>, hash_pair> & inv_idx, bool max) {
    auto entrySize = sizeof(std::pair<char, char>) + sizeof(void*);
...
int main(int argc, char** argv) {
    if (argc != 3) {
        ...
        std::cerr << "\t./bigram_record_all_[NUM_INDEXED].o [expr_idx] [wlname]" << std::endl;
        return EXIT_FAILURE;
    }

    const std::string wlname = std::string(argv[2]);
    ...
    std::string reg_file;
    std::vector<std::string> lines;
    if (!wlname.compare("db_x")) {
        reg_file = "../sampled_db_x_regex.csv";
        lines = read_db_x();
    } else if (!wlname.compare("sys_y")) {
        reg_file = "../regexes_new.txt";
        lines = read_sys_y();
    } else if (!wlname.compare("traffic")) {
        reg_file = "../regexes_traffic.txt";
        lines = read_traffic();
    }
    ...
    std::unordered_map<std::pair<char, char>, size_t, hash_pair> gram_to_idx_english;
    std::vector<std::string> freq_bigram_english;
    for (auto i = 0; i < NUM_INDEXED; i++) {
        gram_to_idx_english.insert({std::make_pair(freq_bigram_english[i].at(0),freq_bigram_english[i].at(1)), i});
    }
    
    start = std::chrono::high_resolution_clock::now();
    auto English_index = build_index(gram_to_idx_english, lines);
    end = std::chrono::high_resolution_clock::now();
    auto English_index_building_time = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);

    std::cout << "English Index building: " << English_index_building_time.count() << " seconds" << std::endl;

    std::ofstream ind_out{"ngram_record_all_script/index_building.csv", std::ios::app};
    ind_out << "bigram," << NUM_INDEXED << "," << bit_index_building_time.count() << "," << bit_index_size << "," << English_index_building_time.count() << std::endl;
    ind_out.close();

    int num_repeat = 10;

    const std::filesystem::path dir_path = "ngram_record_all_script/new_bigram_" + std::to_string(NUM_INDEXED);
    ...
    for (size_t i = 0; i < num_repeat; i++) {
        ...
        expr_r_file << "regex\tfilter_time\tEnglish_time\tdirect_time" << std::endl;
    ...
    r_file << "regex\tfilter_time\tmid_filter_time\tEnglish_time\tmid_English_time\tdirect_time\tmiddirect_time\tfilter\tnum_after_filter\tfilter_english\tnum_after_filter_english\tmatch_num_filter\tmatch_num_English\tmatch_num_direct" << std::endl;

    for (const std::string & r : regexes) {
        auto bitmask = GetBitMask(gram_to_idx, r);
        auto bitmask_english = GetBitMask(gram_to_idx_english, r);
        std::vector<double> elapsed_time_direct;
        std::vector<double> elapsed_time_filter;
        std::vector<double> elapsed_time_English;
        int match_count_English;
        ...
        for (int i = 0; i < num_repeat; i++) {
            ...
            auto result_English = Re2FilterMatch(lines, English_index, gram_to_idx_english, r);

            elapsed_time_direct.push_back(result_direct.first);
            elapsed_time_filter.push_back(result_filter.first);
            elapsed_time_English.push_back(result_English.first);
            expr_out_files[i] << r << "\t" << result_filter.first << "\t" << result_English.first << "\t" << result_direct.first << std::endl;

            match_count_English = result_English.second;
        ...
        std::sort(elapsed_time_English.begin(), elapsed_time_English.end());
        auto ave_invert = std::accumulate(elapsed_time_English.begin(), elapsed_time_English.end(), 0.0) / elapsed_time_English.size();        
        auto mid_ave_invert = std::accumulate(elapsed_time_English.begin()+1, elapsed_time_English.end()-1, 0.0) / (num_repeat-2);
        ...
        r_file << bitmask_english.to_string() << "\t" << StatsAfterFiltering(lines, English_index, gram_to_idx_english, r) << "\t";
        r_file << match_count_filter << "\t" << match_count_English << "\t" << match_count_direct << std::endl;
    }
    r_file.close();
