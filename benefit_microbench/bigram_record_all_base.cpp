...
const static int NUM_INDEXED = 64;
...
int main(int argc, char** argv) {
    ...
    // IMPORTANT: paste the list of bigrams from the results in notebook
    std::vector<std::string> freq_list;
    std::unordered_map<std::pair<char, char>, size_t, hash_pair> gram_to_idx_freq;
    for (size_t i = 0; i < NUM_INDEXED; i++) {
        gram_to_idx_freq.insert({std::make_pair(freq_list[i].at(0), freq_list[i].at(1)), i});
    }
    auto top_freq_idxs = build_index(gram_to_idx_freq, lines);

    // IMPORTANT: paste the list of bigrams from the results in notebook
    std::vector<std::string> bene_list;
    std::unordered_map<std::pair<char, char>, size_t, hash_pair> gram_to_idx_bene;
    for (size_t i = 0; i < NUM_INDEXED; i++) {
        gram_to_idx_bene.insert({std::make_pair(bene_list[i].at(0), bene_list[i].at(1)), i});
    }
    auto top_bene_idxs = build_index(gram_to_idx_bene, lines);

    // IMPORTANT: paste the list of bigrams from the results in notebook
    std::vector<std::string> incr_bene_list;
    std::unordered_map<std::pair<char, char>, size_t, hash_pair> gram_to_idx_incr_bene;
    for (size_t i = 0; i < NUM_INDEXED; i++) {
        gram_to_idx_incr_bene.insert({std::make_pair(incr_bene_list[i].at(0), incr_bene_list[i].at(1)), i});
    }
    auto top_incr_bene_idxs = build_index(gram_to_idx_incr_bene, lines);


    int num_repeat = 10;

    const std::filesystem::path dir_path = "microbench_script/microbench" + std::to_string(NUM_INDEXED);
    ...
    for (size_t i = 0; i < num_repeat; i++) {
        ...
        expr_r_file << "regex\tfilter_time\tbene_filter_time\tincr_filter_time" << std::endl;
    ...
    r_file << "regex\tfilter_time\tmid_filter_time\tbene_filter_time\tmid_bene_filter_time\tincr_filter_time\tmid_incr_filter_time\tfilter\tnum_after_filter\tmatch_num_filter\tmatch_num_bene\tmatch_num_incr_bene" << std::endl;

    for (const std::string & r : regexes) {
        auto bitmask = GetBitMask(gram_to_idx_incr_bene, r);
        ...
        for (int i = 0; i < num_repeat; i++) {
            std::pair<double, int> result_filter = Re2FilterMatch(lines, top_freq_idxs, gram_to_idx_freq, r);
            // std::pair<double, int> result_direct = std::make_pair(0, 0);
            std::pair<double, int> result_inverted = Re2FilterMatch(lines, top_bene_idxs, gram_to_idx_bene, r);
            ...
            //std::pair<double, int> result_direct = Re2DirectMatch(lines, r);
            // std::pair<double, int> result_inverted = Re2InvertedFilterMatch1(lines, inverted_index, r);
            // auto result_inverted = Re2InvertedFilterMatch2(lines, inverted_index, r);
            auto result_direct = Re2FilterMatch(lines, top_incr_bene_idxs, gram_to_idx_incr_bene, r);
        ...
        r_file << bitmask.to_string() << "\t" << StatsAfterFiltering(lines, top_incr_bene_idxs, gram_to_idx_incr_bene, r) << "\t";
