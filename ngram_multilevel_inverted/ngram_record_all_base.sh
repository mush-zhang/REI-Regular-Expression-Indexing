...
num_ngrams_list=( 4 8 16 32 64 96 192 256 512 )
granu_list=( 8 32 64 96 192 256 512 )
grams_list=( bi )
...
for num_ngram in ${num_ngrams_list[*]}; do
    for grams in ${grams_list[*]}; do
        for granu in ${granu_list[*]}; do
            rm -r new_${grams}_${num_ngram}_${granu}
            rm ../${grams}_record_all_${num_ngram}_${granu}.o
...
for num_ngram in ${num_ngrams_list[*]}; do
    for grams in ${grams_list[*]}; do
        for granu in ${granu_list[*]}; do
            ./${grams}_record_all_${num_ngram}.o 0 ${granu}
        done
    done
done
